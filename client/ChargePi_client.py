...
from charge_point.hardware.mfrc522 import MFRC522
...
def get_reader(reader_info):
    if reader_info["is_supported"]:
        if reader_info["reader_model"] == "MFRC522":
            print('NFC Module selected - MFRC522')
            return MFRC522()
        ...
        try:
            if reader is not None:
                ...
                if reader_info["reader_model"] == "MFRC522":
                    try:
                        ...
                        if status == reader.MI_OK:
                            # Зчитати UID картки
                            (status, uidArray) = reader.MFRC522_SelectTagSN()
                            # Продовжити якщо UID отримано
                            if status == reader.MI_OK:
                                print(uidArray)
                                uid = mfrc522UidToString(uidArray)
                            else:
                ...
                if is_full_string(uid):
                    ...
                    print(read_tag_info)
                    handle_request(uid)
                    # Затримка, щоб упевнитись, що картку прибрано
                    time.sleep(4)
            else:
